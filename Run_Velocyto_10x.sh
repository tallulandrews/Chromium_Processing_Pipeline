...
#echo "velocyto run -b $BARCODES -o $VELOCYTO_DIR -u chr --samtools-threads $THREADS --samtools-memory $((1000*$MEMORY_GB)) $BAM $CELLRANGER_GTF"
#velocyto run -b $BARCODES -o $VELOCYTO_DIR -u chr --samtools-threads $THREADS --samtools-memory $((1000*$MEMORY_GB)) $BAM $CELLRANGER_GTF
